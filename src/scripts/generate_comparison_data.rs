 ...
 * I'm having a lot of trouble with the parsing.
 */
fn read_metadata(sequence_database_file_name: &String) -> Result<(usize, (usize, usize))> {
 ...
 * 
 * The main issues I'm having revolve around parsing numeric data from files.
 */
fn main() -> Result<()> {
    ...
    let (_sequence_length, (min_edit_distance, max_edit_distance)) = match read_metadata(&sequence_database_file_name) {
        Ok((a, (b, c))) => (a, (b, c)),
        Err(e) => {
            println!("error reading sequence length: {}", e);
            process::exit(1);
        } // expected ({unknown}, ({unknown}, {unknown})), found (). rust-analyzerE0308
    };
